...
export default function EditarMedicos() {
  ...
  //Estado de usuaro para obtener el id y los datos de la especialidad
  const [especialidadid, setEspecialidad_id] = useState(
    medico?.especialisaciones_id?.toString() || ""
  );
    ...
    const cargarEspecialidades = async () => {
      try {
        ...
        if (resEspecialidades.success) {
          setEspecialidades(resEspecialidades.data); // Asume que los datos vienen en .data
        }
      } catch (error) {
  ...
  const validarTelefono = (telefono) => {
    ...
    if (soloNumeros.length > 10) {
      setErrors({
        ...errors,
        telefono: "El teléfono debe tener máximo 10 dígitos",
      });
      return false;
    }
  ...
  const handleGuardar = async () => {
    ...
    if (!numDocumento)
      nuevosErrores.numDocumento = "El documento es obligatorio";
    ...
    if (!especialidadid)
      nuevosErrores.especialidadid = "La especialidad es obligatoria";
  ...
  return (
    <View style={styles.container}>
      <Text style={styles.titulo}>
        {esEdicion ? "Editar Medicos" : "Nuevo Medicos"}
      </Text>
      ...
      <ScrollView contentContainerStyle={styles.scrollContent}>
        ...
        />
        {errors.numDocumento && (
          <Text style={styles.errorText}>{errors.numDocumento}</Text>
        )}
        ...
        {/*  Picker especialidades  */}
        <View style={styles.pickerContainer}>
          <Picker
            selectedValue={especialidadid} // Corregido: usa el estado
            onValueChange={(itemValue) => setEspecialidad_id(itemValue)} // Corregido: actualiza el estado
            style={styles.input}
          >
            <Picker.Item label="Seleccione una especialidad" value="" />
            {especialidades.map(
              (
                especialidad // Corregido: variable consistente
              ) => (
                <Picker.Item
                  key={especialidad.id}
        ...
        {errors.especialidadid && (
          <Text style={styles.errorText}>{errors.especialidadid}</Text>
        )}
      </ScrollView>
      <TouchableOpacity
        style={styles.boton}
